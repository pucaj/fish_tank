...
public class fun{
     ...
     * @throws IllegalArgumentException
     */
    public static boolean in_bounds(Vec2d bounds, Vec2d point) throws IllegalArgumentException{
        if(bounds.x <= 0 || bounds.y <= 0) throw new IllegalArgumentException("Bounds cannot be negative or equal zero");
        if( point.x <= 0 || point.x >= bounds.x || point.y <= 0 || point.y >= bounds.y ) return false;
        else return true;
    }
