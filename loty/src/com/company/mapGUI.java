...
public class mapGUI extends JPanel implements MouseListener,MouseMotionListener {
    ...
    public mapGUI(Dimension dim, ArrayList<statek> ships, ArrayList<budynek> buildings){
        ...
        this.ships = new ArrayList<statek>(ships);
        this.buildings = new ArrayList<budynek>(buildings);
        this.warning_boxes = new ArrayList<Shape>();
    }
    ...
    public void mouseClicked(MouseEvent e) {
        for( statek s: ships ){
            if(s.getPozycja().distance(s.getPozycja().x,s.getPozycja().y,(double)e.getX(),((double)e.getY())) < 25){
                activeShip = s.id;
                break;
            } else {
                activeShip = -1;
    ...
    private boolean hover(statek s){
        if(!trackable) {
            ...
            if(s.getPozycja().distance(s.getPozycja().x,s.getPozycja().y, mX,mY) < 25){
