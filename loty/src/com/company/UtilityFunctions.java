...
public class UtilityFunctions {
    ...
    public static boolean imageRepresentation;
    ...
    public static Vec2d rand_point_on_rect(Vec2d bounds) throws IllegalArgumentException {
        if (bounds.x <= 0 || bounds.y <= 0)
            throw new IllegalArgumentException("Bounds cannot be negative or equal zero");
        double x, y;
        if (rand_sgn() > 0) {
            if (rand_sgn() > 0) {
                x = 0;
    ...
    public static Vec2d safe_place(double _long, double width, double height, ArrayList<Budynek> budynki) throws IllegalArgumentException {
        ...
        while (!can_place) {
            ...
            for (Budynek b : budynki) {
                if (b.zagrozenie(new Vec2d(x, y), height)) {
                    can_place = false;
                    break;
                } else {
                    can_place = true;
    ...
    public static Vec2d safe_place(Vec2d bounds, double height, ArrayList<Budynek> budynki, boolean arrival) throws IllegalArgumentException {
        ...
        while (!can_place) {
            ...
            for (Budynek b : budynki) {
                if (b.zagrozenie(new Vec2d(point.x, point.y), height)) {
                    can_place = false;
                    break;
                } else {
                    can_place = true;
    ...
    public static double min_to_sec(double min) throws IllegalArgumentException {
        if (min < 0) throw new IllegalArgumentException("Minutes cannot be negative");
        return min * 60;
    }

    public static int hour_to_sec(int hour) throws IllegalArgumentException {
        if (hour < 0) throw new IllegalArgumentException("Hours cannot be negative");
        return hour * 60 * 60;
    }

    public static double sec_to_min(int sec) throws IllegalArgumentException {
        if (sec < 0) throw new IllegalArgumentException("Seconds cannot be negative");
        return sec / 60;
    }
